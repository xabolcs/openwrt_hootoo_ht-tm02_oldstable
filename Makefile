...
QUILT := $(BUILDER)/staging_dir/host/bin/quilt
PROFILE := ht-tm02
#DEVICE_DTS := $(SOC)_$(PROFILE)
DEVICE_DTS := $(DTS)
PACKAGES := iw luci nano -wpad-mini -wpad-basic -wpad-basic-wolfssl wpad-mesh-wolfssl kmod-rt2800-usb luci-app-travelmate travelmate -dnsmasq
...
$(BUILDER): $(BUILDER).tar.xz
	...
	# Apply all patches
	cd $(BUILDER) && patch -p1 < ../0001-ramips-fix-HooToo-HT-TM02-partitions.patch && \
		patch -p1 < ../0002-ramips-add-HooToo-Tripmate-HT-TM01.patch
...
images: $(BUILDER) $(KDIR)/$(PROFILE)-kernel.bin
	...
	cd $(BUILDER) && make image PROFILE="$(PROFILE)" EXTRA_IMAGE_NAME="$(EXTRA_IMAGE_NAME)" PACKAGES="$(PACKAGES)" FILES="/mnt/data/data/DEV/PROJECTS/PRIVATE/routers/imagebuilder-files/files"

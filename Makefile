...
$(KDIR)/$(PROFILE)-kernel.bin: $(BUILDER) linux-include
	...
	cd $(BUILDER) && env PATH=$(PATH) make --trace -C target/linux/$(BOARD)/image $(KDIR)/$(PROFILE)-kernel.bin TOPDIR="$(TOPDIR)" INCLUDE_DIR="$(TOPDIR)/include" TARGET_BUILD=1 BOARD="$(BOARD)" SUBTARGET="$(SUBTARGET)" PROFILE="$(PROFILE)" DEVICE_DTS="$(DTS)"
...
images: $(BUILDER) $(KDIR)/$(PROFILE)-kernel.bin
	...
	ls -hs $(BUILDER)/bin/targets/$(BOARD)/$(SUBTARGET)/openwrt-$(RELEASE)-$(EXTRA_IMAGE_NAME)-*.bin


clean:
	rm -rf openwrt-imagebuilder-$(RELEASE)-$(BOARD)-$(SUBTARGET).Linux-x86_64
	rm -rf openwrt-imagebuilder-$(RELEASE)-$(BOARD)-$(SUBTARGET).Linux-x86_64.xz
	rm -rf linux-include
